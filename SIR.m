...
h = 0.0001;
params.h = h; %input('h : ') ; % time grid step
...
eps = 0.00001; %input('precision : ') ; % precision ε for the gradient algorithm
maxit = 500; %input('maxiter : ') ; % max. no. iterations – gradient algorithm disp('RO data') ;
roin = 1; %input('RO : ') ; % initial value for gradient steplength ρ
bro = 0.6; %0.6; %input('b for RO : ') ; % b parameter for ρ loop
eps1 = 0.00001; %input('precision for RO : ') ; % precision ε1 for steplength ρ
maxro = 20; %input('max for RO : ') ; % max. no. of iterations – ρ loop
...
for iter = 1:maxit
    ...
    if iter == 1
        ...
        for i = 1:n-1
            ...
            %x(i+1) = x(i) + h*F1(params,t(i),x(i),uold(i));
        end
    ...
    for j = 1:n-1
        ...
        %p(i) = p(i+1) - h*F3(params,t(i+1),x(i+1),p(i+1));
    end
    disp('AE solved');
